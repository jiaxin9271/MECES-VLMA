...
class MoLoRN50(nn.Module):
    def __init__(self, ):
        super(MoLoRN50, self).__init__()
        ...
        self.time_transformer = Transformer(heads=8, dim=1024, dropout_attn=0.05, dropout_ffn=0.05, depth=1)
    ...
    def forward(self, x):
        ...
        x = x.reshape(-1, 8, 1024)
        x = self.time_transformer(x)
        return x
    

class MoLoViT(nn.Module):
    def __init__(self, name):
        super(MoLoViT, self).__init__()
        ...
        self.time_transformer = Transformer(heads=8, dim=512, dropout_attn=0.05, dropout_ffn=0.05, depth=1)
